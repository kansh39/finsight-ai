...
with col1:
    company = st.text_input("Company Name", placeholder="e.g. Infosys, Tesla, Apple, TCS")
with col2:
    ticker = st.text_input("Stock Ticker", placeholder="e.g. INFY, TSLA, AAPL")
with col3:
...
if qcol1.button("Infosys"):
    company, ticker = "Infosys", "INFY"
    analyze_btn = True
if qcol2.button("TCS"):
    company, ticker = "TCS", "TCS.NS"
    analyze_btn = True
if qcol3.button("Tesla"):
    company, ticker = "Tesla", "TSLA"
    analyze_btn = True
if qcol4.button("Apple"):
    company, ticker = "Apple", "AAPL"
    analyze_btn = True
if qcol5.button("Google"):
    company, ticker = "Google", "GOOGL"
    analyze_btn = True
if qcol6.button("Microsoft"):
    company, ticker = "Microsoft", "MSFT"
    analyze_btn = True


def generate_pdf(company, ticker, fin, report_text):
    class PDF(FPDF):
        def header(self):
...
if analyze_btn:
    if not company or not ticker:
        ...
        s1 = col1.status("Research Agent", expanded=False)
        s2 = col2.status("Financial Agent", expanded=False)
        s3 = col3.status("Sentiment Agent", expanded=False)
        s4 = col4.status("Report Agent", expanded=False)

        with st.spinner(f"Analyzing {company}... please wait..."):
            try:
                with s1:
                    st.write("Searching latest news...")
                with s2:
                    st.write("Pulling live stock data...")
                with s3:
                    st.write("Analyzing market sentiment...")
                with s4:
                    st.write("Writing investment report...")
                ...
                try:
                    pdf_bytes = generate_pdf(company, ticker, fin, result["report"])
